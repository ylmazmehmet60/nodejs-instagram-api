...
const PolarisEnvelopeEncryption = {
  encrypt: function(a, c, d, e) {
    return new Promise((resolve, reject) => {
      ...
      crypto.subtle.generateKey(keyAlgorithm, true, ['encrypt', 'decrypt'])
        .then((key) => {
...
function encPassword(version, public_key, pwd, timestamp, format_version) {
  return new Promise((resolve, reject) => {
    PolarisEnvelopeEncryption.encrypt(version, public_key, Buffer.from(pwd), timestamp)
      .then((encryptedData) => {
        const base64EncryptedData = encryptedData.toString('base64');
        const formattedPassword = formatPassword(base64EncryptedData.toString(), timestamp, format_version);
